...
import psycopg2
from utils.bot_utils import has_role, get_global, get_id_guild, get_postgres_credentials
        ...
        try:
            ...
            while row is not None:
                ...
                if member is not None:
                    if sync_levels:
                        ...
                        cur2.execute(
                            'update member set level=%s, last_update=now(), member_left=false where member_id=%s',
                            (level, db_member_id))
                    else:
                        cur2.execute(
                            'update member set last_update=now(), member_left=false where member_id=%s',
                            (db_member_id,))
            ...
            for i, member in enumerate(guild_members):
                ...
                if has_role(member, 'booster_id') is False and member.bot is False:
                    await member.add_roles(discord.Object(newbie_role_id))
                cur.execute(
                    "insert into member (member_id, username, level) values (%s, %s, %s)",
